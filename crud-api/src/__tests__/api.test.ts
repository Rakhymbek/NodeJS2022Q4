...
describe("Simple-crud-api tests", () => {
  ...
  it("POST-request should create a new user", async () => {
    await request(server).post("/api/users").send(mockUsersData).then((res) => {
      expect(res.body.username).toBe(mockUsersData.username);
      expect(res.body.age).toBe(mockUsersData.age);
      expect(res.body.hobbies.toString()).toBe(mockUsersData.hobbies.toString())
  ...
  it("GET-request should return created user by id", async () => {
    ...
    await request(server).get(`/api/users/${user.body.id}`).then((res) => {
      expect(res.body.username).toBe(mockUsersData.username);
      expect(res.body.age).toBe(mockUsersData.age);
      expect(res.body.hobbies.toString()).toBe(mockUsersData.hobbies.toString());
  ...
  it("PUT-request should update user with the same id", async () => {
    const user = await request(server).post("/api/users").send(mockUsersData);
    const updatedUser = await request(server).put(`/api/users/${user.body.id}`).send(mockUpdatedUsersData);
    await request(server).put(`/api/users/${user.body.id}`).send(mockUpdatedUsersData).then((res) => {
      ...
      expect(res.body.hobbies.toString()).toBe(updatedUser.body.hobbies.toString());
      expect(res.statusCode).toBe(200);
    });
  });
  ...
  it("GET-request should return 404 and error message if the route is incorrect", async () => {
    ...
    await request(server).get(`/api/user`).then((res) => {
